...
def test_get_relative_path(path, expected):
    ...
    class Settings(object):
        ...
        DATAS_DIR = "/foo/project"

    handler.settings = Settings()

    assert handler.get_relative_template_path(path) == expected
    assert handler.get_relative_asset_path(path) == expected
    assert handler.get_relative_data_path(path) == expected
